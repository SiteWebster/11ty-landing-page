...
export async function POST(
  ...
  const results = steps.filter((s) => s.type === "results");
  ...
  if (results.length !== 1) {
    errors.push(`Expected exactly 1 results step, found ${results.length}`);
  }
  ...
  // If the latest version is already published, create a new immutable version
  let publishVersionId: string;
  if (flow.publishedVersionId === latestVersion.id) {
    // Already published version — nothing new to publish
  ...
  publishVersionId = publishedVersion.id;
  ...
  return NextResponse.json({ success: true, publishedVersionId: publishVersionId });
}
